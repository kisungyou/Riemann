...
#' 
#' @section Default Method : Finite Difference Approximation
...
#' @concept basic
#' @export
riem.curvedist <- function(riemobj1, riemobj2, vect=NULL, geometry=c("intrinsic","extrinsic"), ...){
  ## PREPARE : EXPLICIT
  DNAME1 = paste0("'",deparse(substitute(riemobj1)),"'")
  DNAME2 = paste0("'",deparse(substitute(riemobj2)),"'")
  if (!inherits(riemobj1,"riemdata")){
    stop(paste0("* riem.curvedist : input ",DNAME1," should be an object of 'riemdata' class."))
  }
  if (!inherits(riemobj2,"riemdata")){
    stop(paste0("* riem.curvedist : input ",DNAME2," should be an object of 'riemdata' class."))
  }
  if (!all(riemobj1$name==riemobj2$name)){
    stop("* riem.curvedist : two inputs are from different manifolds.")
  }
  if (!all(riemobj1$size==riemobj2$size)){
    stop("* riem.curvedist : two inputs are of different size.")
  }
  if (length(riemobj1$data)!=length(riemobj2$data)){
    stop("* riem.curvedist : two inputs should be of same size.")
  }
  ...
  if (length(myvect)!=N){
    stop("* riem.curvedist : length of 'vect' is not matching to that of the data.")
  }
  ...
  outdist = switch(mymethod,
                   lp = basic_curvedist_lp(riemobj1$name, mygeometry, riemobj1$data, riemobj2$data, myvect, myp))
  return(outdist)
}