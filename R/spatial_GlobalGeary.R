...
riem.geary <- function(riemobj, W, alternative=c("two.sided","greater","less"), ...){
  ...
  if (!inherits(riemobj,"riemdata")){
    stop(paste0("* riem.geary : 'riemobj' should be an object of 'riemdata' class."))
  }
  N = length(riemobj$data)
  
  # weight
  if (!spatial_check_W(W, N)){
    stop("* riem.geary : 'W' is not a valid weight matrix. Please see the documentation.")
  }
  ...
  cpprun = spatial_geary_global(riemobj$name,
                                par_geom,
                                riemobj$data,
                                W,
                                par_ntest)
